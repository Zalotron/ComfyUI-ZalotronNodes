...
class LatentContextConnectionLTX:
    ...
    DESCRIPTION = (
        ...
        "clip_2 is the end: its first 25 frames encode to 4 latents and the last "
        "3 of those are written at the end, though the noise mask only freezes "
        "the final one. At least one clip must be connected."
    )
    ...
    def connect(self, latent, vae, clip_1=None, clip_2=None):
        ...
        if clip_1 is not None:
            head = self._encode(vae, clip_1[:HEAD_FRAMES], width, height)
            n = min(HEAD_LATENTS, head.shape[2], count)
            samples[:, :, :n] = head[:, :, :n]

        tail_n = 0
        if clip_2 is not None:
            tail = self._encode(vae, clip_2[:TAIL_FRAMES], width, height)
        ...
        if tail_n:
            mask[:, :, count - min(TAIL_FROZEN, tail_n):] = 0.0
